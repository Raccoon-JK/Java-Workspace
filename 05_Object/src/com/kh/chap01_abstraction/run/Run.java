...
public class Run {
	 ...
	 * 
	 * 5. 추상화 과정
	 * 	1) 내가 구현하고자 하는 프로그램에서 필요한 객체들을 생각해볼것 ex) 학생관리 프로그램  -> '학생'객체 : 홍길동 학생, 민경민학생 등등..
	 ...
	 * 			민경민이라는 학생객체를 만들기 위해선 => String name2 = "민경민", int age2 = 30; double height1 = 188.8;
	 * 										=> String name3 = ...;
	 * 			=> 관리하고자하는 학생들이 늘어나면 늘어날수록 생성해야할 갯수도 엄청나게 증가하게됨. 즉 관리하기가 굉장히 힘듬.
	 * 
	 * 		2) "배열"을 이용해서 프로그래밍을 하게 된다면 ?
	 * 			배열 : 하나의 자료형인 여러개의 값들을 보관할수 있음.(단, 동일한자료형일시)
	 ...
	 * 			=> 변수를 사용하는 것보다는 나아보임.
	 * 			=> 하지만 관리하는 학생중 한명이 전학을 가게되는경우 해당 인덱스에있는 모든 키, 나이에 대한정보도 제거를 해줘야함.
	 ...
	 * 			구조체 : 한번에 여러개의 자료형을 보관할수 있는 배열같은 형태.
...
//		// 직접적으로 접근해서 값 대입
		hong.name = "홍길동";
		hong.age = 20;
		hong.height = 166.7; // 필드 접근하고자한다면 .을통해 가져올수있음(단, 접근제한자가 public인 경우)
		 ...
		 * => 외부에서 함부로 값을 변질시키거나 조회 권한이 없는 사람이 값을 쉽게 가져올수 있는 문제 발생
		 * >>> 캡슐화로 해결.
		 * 객체지향의 설계원천중 하나가 바로 정보은닉이며 그 기술 중 하나가 캡슐화
		 */
	}
