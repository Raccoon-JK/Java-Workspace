...
public class Run {
	...
	public static void main(String[] args) {
		...
		// 저장 순서 유지 x , 중복 저장 o -> 동일 객체로 판단되지 않기 떄문.
		// 값이 추가될때 equals(), hashCode()로 비교 후 둘다 결과가 true면 동일객체로 판단.
		
		// HashSet에 객체를 담을 떄 내부적으로 equals()로 비교 + hashCode()값이 일치하는지도 비교
		// equals() 한 결과가 true이고 hashCode()값이 일치하면 => 동일객체로 판단(중복저장 x)
		
		// Object클래스에 있는 equals() : 두 객체의 주소값을 가지고 비교해서 일치하면 true / 일치하지않으면 false
		...
		ArrayList<Student>list = new ArrayList<>(hs2); // hs2에 담겨있는 객체들이 추가된채로 리스트 생성됨.
