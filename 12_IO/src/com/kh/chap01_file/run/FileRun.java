...
public class FileRun {
	...
	public static void main(String[] args) {
		...
		// 2. 경로 지정을 한 파일명(C:\\aaa\\test.txt)을 제시해서 생성하기=> 이 때 존재하는 경로를 제시해야함.
			File file2 = new File("C:\\aaa\\tset.txt");
			file2.createNewFile();
		 ...
		 * 스트림의 구분
		 * - 통로의 사이즈:
		 * 		바이트 스트림 : 1byte짜리가 이동할 수 있을정도의 사이즈(좁은통로) => 입력(XXXInputStream)/출력(XXXOutputStream)
		 * 		문자 스트림 : 2byte짜리가 이동할 수 있을정도의 사이즈(넓은 통로) => 입력(XXXReader) / 출력(XXXWriter)
		 * 
		 * - 외부매체와의 직접적인 연결여부 :
		 * 		기반스트림 : 외부매체와 직접적으로 연결되는 통로
		 * 		보조스트림 : 기반스트림만으로 부족한 성능을 향상시켜주는 용도의 스트림(직접적으로 외부매체와 연결할수 없음, 즉 단독사용이 불가)
		 * 				  (보조해주는 기능 : 기능향상, 자료형 변환, 객체단위의 입출력 보조 등등)
