...
public class PolyRun {
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * 1. 부모타입 래퍼런스 변수로 모든 자식 객체를 다루는것 = 당연히 가능하다
		 */
		...
		Child1 c1 = new Child1(); // 기본생성자에 슈퍼생성자호출하는 구문이 항상 있음. 따라서 부모객체사용가능.
		c1.printChild1();
		((Parent)c1).printParent();
		// c1 레퍼런스로 Parent(부모), Child1(자식)에 있는 필드나 메서드 둘다 접근가능.
		// c1레퍼런스변수가 Parent에 있는 메소드에 접근시 자동으로 형변환된채 진행됨.
		// 즉 자식에서 부모클래스 자동형변화(업캐스팅). -> 작은것에서 큰것으로 형변환 된것과 같은 이치
		 ...
		 * Parent형으로 '자동형변화'이 되고 있기 때문에
		 * 상속 구조에서는 클래스 형변환이 가능하다.
		 */
		
		p2.printParent();// p2라는 페퍼런스 변수로는 Parent에만 접근이 가능.
		((Child1)p2).printChild1(); // 단, 자식객체로 접근하고싶을때 Child형으로 강제형변환후 Child1에 접근가능.
		...
		for(int i = 0; i < arr.length; i++) {
			...
			// 현재 참조하고있는 레퍼런스 변수가 실제로 어떤 클래스(객체)타입을 참조하고 있는지 확일할때 사용
...
//			}
			 ...
			 * 단, 참조하고 있는 자식클래스에해당 메소드가 오버라이딩 되어 있다면
			 * 프로그램을 시작할때는 동적으로 그 자식클래스의 오버라이딩된 메소드를 찾아서 실행하게됨
			 * (ctrl 누로고 들어가보면 parent클래스의 메소드를 실행한다고 했지만(정적바인딩),
