...
public interface Basic {
	 ...
	 * 
	 * - 상수필드와 추상메소드로만 이루어진 추상 클래스이 변형체
	 ...
	 * 인터페이스는 인스턴스 변수, 메소드 생성이 불가능하며 모든 변수는 상수필드, 모든 메소드는 정의되어있다.
	 *
	 * - 존재하는 목적이 다르다.
	 * 	추상클래스는 추상클래스를 상속받아서 기능을 이용하고, 클래스를 확장하는데 목적이 있다.
	 *  인터페이스는 클래스의 기능(함수) 구현을 강제하기 위해 사용된다. 즉 구현을 강제함으로써 구현객체의 동일한 동작을 보장할수 있다.
	 *  
	 *  extends 와 implements
	 *  	- 클래스간에 상속 관계일떄 : 클래스명 extends 부모클래스명
	...
	void sleep();
	// 인터페이스에서는 추상메소드만 정의할 수 있기 떄문에 메소드의 몸통부 작성 불가능하며
