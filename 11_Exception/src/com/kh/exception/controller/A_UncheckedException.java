...
public class A_UncheckedException {
	 ...
	 * - ArrayIndexOutOfBoundException : 배열의 부적절한 인덱스로 접근할때 발생하는 예외
...
//			System.out.println("0으로 나눌수 없습니다.");
	...
	public void method2() {
		...
		try {
			int num = sc.nextInt(); // InputMiasmatchException : 정수 이외의 값을 입력시 발생함.
		
			System.out.println("나눗셈 연산 결과 : "+(10) / num); // 예외발생할수 있음.
		}catch(ArithmeticException e){
			System.out.println("0으로 나눌수 없습니다.");
	...
	public void method3() {
		...
		if(size > 0) { // 방법 1. if문으로 size값이 양수값일때만 생성하도록 제어.
			int [] arr = new int[Math.abs(size)]; // 방법2. 항상 양의 정수값이 들어갈수 있도록 abs메서드 사용
		}
...
//					+ "인덱스로 접근한것 같습니다.");
//		} // 다중 catch블럭이 줄어들었음. 정확히 어떤 에러인지 알수 없음.
		...
		try {
			...
			System.out.println("예외가 발생하긴 했네요. 근데 배열의 크기를 음수로 입력했거나, 부적절한"
					+ "인덱스로 접근한것 같습니다.");
		} // 다중 catch에서는 블럭의 순서가 중요함. => 범위가 작은 자식 타입의 예외 클래스를 먼저 기술해야함.
