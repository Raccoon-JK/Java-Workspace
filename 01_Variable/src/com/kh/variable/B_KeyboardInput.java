...
public class B_KeyboardInput {
	...
	public void inputTest1() {
		...
		String name = sc.nextLine();// 사용자가 엔터까지 입력할 때 까지 대기상태를 유지
		//nextLine() : 사용자가 입력한 값 중 개행이 있을경우 공백에 무관하게 개행 이전까지만 출력해준다.
		//			   특히 사용자가 입력한 값에 공백이 포할될 여지가 있는경우 안전하게 nextLine()함수를 쓰자.
	 ...
	 * 
	 *  > 주의사항
	 *  1) sc.nextXXX()메소드 뒤에 sc.nextLine()메소드가 와야할 경우 sc.nextLine()메소드를 한번 더 실행시켜줘서 버퍼에 남아있는 엔터를 빼줘야 한다.
	 *  
	 *  2) '문자'값을 입력받아야할 경우
	 *  	sc.nextLine()메소드를 통해 우선 문자열을 입력받고, 그뒤에 .charAt(인덱스)메소드를 통해 문자를 하나만 추출한다.
	 */
